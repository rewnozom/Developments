...
from summerize_extract.Extractorz import ExtractorWorker, ReverseCSVEx, ReverseMarkdownEx
...
class SettingsManager(QObject):
    ...
    def _normalize_paths(self) -> None:
        ...
        # Normalize other relevant sections...
    ...
    def remove_setting(self, section: str, key: str) -> None:
        ...
        if isinstance(section_dict, dict) and key in section_dict:
            del section_dict[key]  # Remove the specified key from the section dictionary
            self._save_settings()  # Save changes to the settings file
            self.settings_changed.emit()  # Emit signal to notify other components
...
class EntryRunFrame(QFrame):
    ...
    def run_command(self):
        ...
        from summerize_extract.Extractorz import CSVEx, MarkdownEx
...
class App(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.scrollable_textbox_frame = ScrollableTextboxFrame(self.settings_manager, self)
        self.tabview_frame = TabViewFrame(self.settings_manager, self)
        self.checkbox_frame = CheckboxFrame(self)
        self.entry_run_frame = EntryRunFrame(self.settings_manager, self)
        
        # Create tab widget for right panel
        self.right_tab_widget = QTabWidget()
        self.radiobutton_frame = RadiobuttonFrame(settings_manager=self.settings_manager, parent=self)
        self.quick_paste_frame = QuickPasteFrame(self.settings_manager, self)
        
        # Add frames to tab widget
        self.right_tab_widget.addTab(self.radiobutton_frame, "Standard Extraction")
        self.right_tab_widget.addTab(self.quick_paste_frame, "Quick Paste")
        ...
        # Layout the components with proper sizing
        self.main_layout.setSpacing(20)
        self.main_layout.setContentsMargins(10, 10, 10, 10)
        
        # Column 0: Sidebar
        self.main_layout.addWidget(self.sidebar_frame, 0, 0, 4, 1)
        
        # Column 1: Scrollable Textbox Frame
        self.main_layout.addWidget(self.scrollable_textbox_frame, 0, 1, 1, 2)
        
        
        # Column 3: Settings and File Specific Frames (inside TabViewFrame)
        self.main_layout.addWidget(self.tabview_frame, 0, 3, 2, 1)
        
        # Column 4: Control Panel with Tabs
        self.main_layout.addWidget(self.right_tab_widget, 0, 4, 2, 1)
        
        # Column 5: Checkbox Frame
        self.main_layout.addWidget(self.checkbox_frame, 2, 4, 1, 1)
        
        # Bottom row: Entry and Run Controls spanning multiple columns
        self.main_layout.addWidget(self.entry_run_frame, 3, 1, 1, 4)
        
        # Configure column stretching
        self.main_layout.setColumnStretch(0, 1)  # Sidebar
        self.main_layout.setColumnStretch(1, 2)  # Scrollable Textbox
        self.main_layout.setColumnStretch(2, 1)  # Slider and Progress
        self.main_layout.setColumnStretch(3, 1)  # Settings Frame
        self.main_layout.setColumnStretch(4, 1)  # Control Panel
        self.main_layout.setColumnStretch(5, 1)  # Checkbox Frame
        
        # Configure row stretching
        self.main_layout.setRowStretch(0, 2)  # Top row
        self.main_layout.setRowStretch(1, 1)  # Middle row
        self.main_layout.setRowStretch(2, 1)  # Lower middle row
        self.main_layout.setRowStretch(3, 1)  # Bottom row

    def setup_connections(self):
        """Setup signal connections between components with proper error handling"""
        try:
            # Connect settings-related signals
            self.scrollable_textbox_frame.setting_changed.connect(self.handle_setting_change)
            self.tabview_frame.preset_changed.connect(self.handle_preset_change)

            # Connect theme and scaling signals
            self.sidebar_frame.appearance_mode_changed.connect(self.handle_appearance_change)

            # Connect quick paste frame signals
            self.quick_paste_frame.extraction_started.connect(
                lambda: self.handle_extraction_state("started"))
            self.quick_paste_frame.extraction_finished.connect(
                lambda: self.handle_extraction_state("finished"))
            
            # Connect checkbox signals
            self.checkbox_frame.extract_csv_changed.connect(
                lambda state: self.handle_extraction_option_change('csv', state))
            self.checkbox_frame.extract_markdown_changed.connect(
                lambda state: self.handle_extraction_option_change('markdown', state))

            # Connect run signals
            self.entry_run_frame.run_triggered.connect(self.handle_run_command)
    ...
    def handle_extraction_state(self, state: str):
        """Handle extraction state changes for both standard and quick paste extractions"""
        try:
            if state == "started":
                # Disable interactive elements during extraction
                self.right_tab_widget.setEnabled(False)
                self.entry_run_frame.setEnabled(False)
                self.checkbox_frame.setEnabled(False)
                
                # Optional: Show a statusbar message or update UI
                if hasattr(self, 'statusBar'):
                    self.statusBar().showMessage("Extraction in progress...")
                    
            elif state == "finished":
                # Re-enable interactive elements
                self.right_tab_widget.setEnabled(True)
                self.entry_run_frame.setEnabled(True)
                self.checkbox_frame.setEnabled(True)
                
                # Clean up status message
                if hasattr(self, 'statusBar'):
                    self.statusBar().showMessage("Extraction completed", 3000)
                    
            # Update the UI to reflect the current state
            QApplication.processEvents()
            
        except Exception as e:
            print(f"Error handling extraction state: {str(e)}")
            QMessageBox.warning(
                self,
                "State Change Error",
    ...
    def handle_setting_change(self, section: str, key: str, value: str):
        ...
        # Implement any necessary UI updates or logic based on the setting change

    def handle_preset_change(self, preset_name: str, files: list):
        """Handle changes to presets"""
        print(f"Preset '{preset_name}' changed - {len(files)} files")
        # Update relevant UI components or perform actions based on preset changes

    def handle_appearance_change(self, mode: str):
        """Handle appearance mode changes"""
        # Update application theme based on the selected appearance mode
        AppTheme.apply_theme(QApplication.instance())

    def handle_scaling_change(self, scale: str):
        """Handle UI scaling changes"""
        # Implement UI scaling logic here, e.g., adjust font sizes, widget sizes, etc.
        try:
            scaling_factor = float(scale.strip('%')) / 100.0
            font_size = max(int(10 * scaling_factor), 8)  # Ensure font size doesn't go below 8
            font = Fonts.get_default(font_size)
            QApplication.instance().setFont(font)
            print(f"UI scaling changed to {scale}")
        except ValueError:
            print(f"Invalid scaling value: {scale}")
    ...
    def handle_extraction_option_change(self, extraction_type: str, enabled: bool):
        ...
        if extraction_type == 'csv':
            self.entry_run_frame.csv_group.setVisible(enabled)
        elif extraction_type == 'markdown':
            self.entry_run_frame.markdown_group.setVisible(enabled)

    def handle_progress_change(self, value: float):
        """Handle progress updates"""
        print(f"Progress updated: {value:.2%}")
        # Update any progress-dependent UI elements or perform actions based on progress

    def handle_segment_change(self, segment: str):
        """Handle changes in the segmented button selection"""
        print(f"Segment changed to: {segment}")
        # Implement any logic based on the selected segment

    def handle_run_command(self):
        """Handle run command"""
        # Implement main run logic here, possibly coordinating multiple components
        print("Run command triggered")
        # Example:
        # runner = ExtractionRunner(self.settings_manager)
        # runner.start()
