...
# Corrected import statements for _phone_extr_v_01.py
...
from android.GUI_Constants_and_Settings import *
...
from android.FileChooserModeSection import FileChooserModeSection
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Initialize file chooser mode
        self.file_chooser_mode = self.settings_manager.get_setting("general", "file_chooser_mode", "Enhanced")
        
        # Uppdatera GuiConstants baserat på inställningar
        self.initialize_gui_constants()
        
        self.setup_ui()
        QTimer.singleShot(100, self.initialize_components)

    def initialize_gui_constants(self):
        """Uppdatera GuiConstants baserat på inställningar."""
        self.mobile_mode = self.settings_manager.get_setting("gui", "MOBILE_MODE", False)
        GuiConstants.MOBILE_MODE = self.mobile_mode
        GuiConstants.TOUCH_SCROLL = GuiConstants.TOUCH_SCROLL and self.mobile_mode
        GuiConstants.KINETIC_SCROLLING = GuiConstants.KINETIC_SCROLLING and self.mobile_mode
        GuiConstants.SMOOTH_SCROLL = GuiConstants.SMOOTH_SCROLL and self.mobile_mode
        GuiConstants.BUTTON_MIN_HEIGHT = 48 if self.mobile_mode else 32
        GuiConstants.SCROLL_BAR_WIDTH = 0 if self.mobile_mode else 12
        GuiConstants.SPACING = 12 if self.mobile_mode else 8
        GuiConstants.SCROLL_SPEED = 50 if self.mobile_mode else 30
        GuiConstants.MOMENTUM_DURATION = 1000 if self.mobile_mode else 0
        GuiConstants.TOUCH_FEEDBACK = True if self.mobile_mode else False
    ...
    def setup_ui(self):
        ...
        # Add file chooser mode section
        self.file_chooser_mode_section = FileChooserModeSection(initial_mode=self.file_chooser_mode)
        self.file_chooser_mode_section.mode_changed.connect(self.change_file_chooser_mode)
        main_layout.addWidget(self.file_chooser_mode_section)
    ...
    def change_file_chooser_mode(self, mode: str):
        """Handle changing the file chooser mode."""
        self.file_chooser_mode = mode
        self.settings_manager.update_setting("general", "file_chooser_mode", mode)
        print(f"MainWindow: File chooser mode changed to {mode}")  # Debug
