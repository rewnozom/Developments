...
from PySide6.QtCore import (
    ...
    QPoint, QPropertyAnimation, QEasingCurve, QEvent, QSize
)
...
class ZoomableScrollArea(QScrollArea):
    """ScrollArea with pinch-to-zoom functionality"""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setHorizontalScrollBarPolicy(Qt.ScrollBarAsNeeded)
        self.setVerticalScrollBarPolicy(Qt.ScrollBarAsNeeded)
        
        # Aktivera touch och gesturer
        self.viewport().setAttribute(Qt.WA_AcceptTouchEvents)
        
        # Zoom-relaterade variabler
        self._zoom_factor = 1.0
        self._min_zoom = 0.5
        self._max_zoom = 3.0
        
        # Aktivera pinch gesture
    ...
    def gestureEvent(self, event):
        ...
        if pinch:
            ...
            # Begränsa zoom inom rimliga gränser
    ...
    def _update_zoom(self):
        # Uppdatera storleken på innehållet
        if self.widget():
    ...
    def setup_zoom_controls(self):
        # Skapa zoom-kontroller
        self.zoom_frame = QFrame(self)
        self.zoom_layout = QHBoxLayout(self.zoom_frame)
        
        zoom_in_btn = QPushButton("+", self.zoom_frame)
        zoom_out_btn = QPushButton("-", self.zoom_frame)
        reset_zoom_btn = QPushButton("Reset", self.zoom_frame)
        ...
        self.zoom_layout.addWidget(zoom_in_btn)
        self.zoom_layout.addWidget(zoom_out_btn)
        self.zoom_layout.addWidget(reset_zoom_btn)
        
        # Placera kontrollerna i nedre högra hörnet
        self.zoom_frame.setGeometry(
            self.width() - 150, self.height() - 60, 140, 50
        )
        
        # Anslut knapparnas signaler
    ...
    def resizeEvent(self, event):
        ...
        # Uppdatera position för zoom-kontrollerna
        self.zoom_frame.setGeometry(
            self.width() - 150, self.height() - 60, 140, 50
        )
...
class FileSpecificSection(QFrame):
    ...
    def setup_ui(self):
        ...
        # Connect key press events for deletion
    ...
    def add_specific_files(self):
        """Add multiple files to specific files list"""
        files, _ = QFileDialog.getOpenFileNames(
            self,
            "Select Files",
            self.settings_manager.get_setting("paths", "base_dir", ""),
    ...
    def add_specific_folder(self):
        """Add all files from a folder to specific files list"""
        folder = QFileDialog.getExistingDirectory(
            self,
            "Select Folder",
            self.settings_manager.get_setting("paths", "base_dir", "")
    ...
    def remove_selected_specific_files(self):
        """Remove selected files from specific files list"""
    ...
    def update_specific_files(self):
        """Update the specific files in settings"""
    ...
    def remove_selected_preset_files(self):
        """Remove selected files from preset files list"""
    ...
    def update_preset_files(self):
        """Update the preset files in settings"""
        preset_name = self.preset_combo.currentText()
        if not preset_name:
            return
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.main_layout.addWidget(self.entry_run_frame, stretch=0)
